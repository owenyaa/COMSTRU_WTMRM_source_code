function residual=cal_residual2(parameter_a)
global index_global N_dof Tdata N_harm N_w0 index  R_dof %w
% amplification=1;
...
residual(N_dof+1,:)=sum(Harm_parameter_a(:,1))-A1;
residual(N_dof+2,:)=sum(Harm_parameter_a(:,3))-A2;
residual(N_dof+3,:)=sum(vector_w(:,1).*Harm_parameter_a(:,2));
residual(N_dof+4,:)=sum(vector_w(:,1).*Harm_parameter_a(:,4));


for i=1:R_dof
    norm_all(i)=norm(residual(i,:));
end
for i=1:R_dof
    weight_norm_all(i)=max(norm_all)/norm_all(i);
    residual(i,:)=weight_norm_all(i)*residual(i,:);
end

%% 原始残差不放大，位移约束和速度约束要放大
% residual(3:R_dof,:)=residual(3:R_dof,:).*weight_norm_all(3:6)';
...
residual(N_dof1+1:2*N_dof1-4,:)=residual(N_dof1+1:2*N_dof1-4,:).*weight_norm_all(1:2)';
residual(2*N_dof1-3:2*N_dof1-2,:)=0;
residual(2*N_dof1-1,:)=weight_norm_all(5)*sum(index_global_w1(:,1).*Harm_parameter_a(:,2));
residual(2*N_dof1,:)=weight_norm_all(6)*sum(index_global_w1(:,1).*Harm_parameter_a(:,4));
...
residual(2*N_dof1+1:3*N_dof1-4,:)=residual(2*N_dof1+1:3*N_dof1-4,:).*weight_norm_all(1:2)';
residual(3*N_dof1-3:3*N_dof1-2,:)=0;
residual(3*N_dof1-1,:)=weight_norm_all(5)*sum(index_global_wd(:,1).*Harm_parameter_a(:,2));
residual(3*N_dof1,:)=weight_norm_all(6)*sum(index_global_wd(:,1).*Harm_parameter_a(:,4));
...
for i=1:2*N_harm*N_dof
    ...
    residual(N_dof1*(i+N_w0)+1:N_dof1*(i+N_w0+1)-4,:)=residual(N_dof1*(i+N_w0)+1:N_dof1*(i+N_w0+1)-4,:).*weight_norm_all(1:2)';
    if i<N_harm*N_dof
        residual(N_dof1*(i+N_w0+1)-3,:)=weight_norm_all(3)*mod(i,2);
        residual(N_dof1*(i+N_w0+1)-2,:)=0;
    else
        residual(N_dof1*(i+N_w0+1)-3,:)=0;
        residual(N_dof1*(i+N_w0+1)-2,:)=weight_norm_all(4)*mod(i,2);
    end
    if mod(i,2)==1
        residual(N_dof1*(i+N_w0+1)-1,:)=0;
        residual(N_dof1*(i+N_w0+1),:)=0;
    else
        residual(N_dof1*(i+N_w0+1)-1,:)=weight_norm_all(5)*sum(sum(sensitivity_parameter_a(:,1:2).*[vector_w(:,1),vector_w(:,1)]));
        residual(N_dof1*(i+N_w0+1),:)=weight_norm_all(6)*sum(sum(sensitivity_parameter_a(:,3:4).*[vector_w(:,1),vector_w(:,1)]));
    end
end
